...
\section{Einleitung}\label{sec:Einleitung}
	...
	\subsection{Aufbau der Arbeit}\label{sec:EinleitungAufbau}
		In Abschnitt 2 werden die nötigen Grundbegriffe zum Verständnis abgesteckt.
		...
		In den Letzten Abschnitten sind noch Code-Snippets, Anhänge und die Eidesstattliche Erklärung.
...
\section{Auswahl Lösung anhand Anforderungen}\label{sec:AuswahlLösungAnhandAnforderungen}
	...
	\subsection{Provider}\label{sec:AuswahlLösungProvider}
		...
		\subsubsection{DMZ Hochschulrechenzentrum}\label{sec:AuswahlLösungDMZHochschulrechenzentrum}
			Die Schwierigkeit eine \nameref{itm:VirtuelleMaschine} zum gewünschten Zweck in der DMZ des Hochschulrechenzentrum einzurichten, ist die Tatsache das keinerlei Rückschlüsse auf die Hochschule gezogen werden dürfen. Ansonsten könnten die Adressen der Hochschule mit Spam-Mails überflutet werden. Außerdem soll das \nameref{itm:OpenRelay} als Mailserver wirken, der unbewusst falsch konfiguriert wurde. Eine Verbindung zur Hochschule könnte die Glaubwürdigkeit des \nameref{itm:OpenRelay} einschränken.
			
			Abgesehen davon wurde der Antrag über eine VM im Rechenzentrum der Hochschule abgelehnt. Das RZ war der Meinung das es aus sicherheitsrelevanten Gründen für dieses Projekt keine \nameref{itm:VirtuelleMaschine} zur Verfügung stellen kann. Darüber hinaus besitzt das Rechenzentrum nur IP-Adressen aus dem HRW-Adressraum, damit könnten diese auf Blacklists landen und den Mailverkehr der Hochschule einschränken oder blockieren. Sofern von der VM Spam-Mails nach außen gelangen.
		...
		\subsubsection{Externe Provider}\label{sec:AuswahlLösungExterneProvider}
			Nach einem Leistungsvergleich von unterschiedlichen Hosting-Providern ist die Entscheidung auf \textsf{Netcup} gefallen, da das Preisleistungsangebot für eine \nameref{itm:VirtuelleMaschine} die Anforderungen erfüllt hat.
			Hier ist eine \nameref{itm:VirtuelleMaschine} inklusive Domain nötig um darauf die Konfiguration für Eingangsmailserver und \nameref{itm:OpenRelay} aufbauen zu können.
...
%	TODO
	...
	\subsection{Provider Registrierung}\label{sec:ProviderRegistrierung} %Mario
		Wie bereits in \autoref{sec:AuswahlLösungProvider} erwähnt, wird die benötigte \nameref{VirtuelleMaschine} für die Umsetzung des Projekts bei einem externen Provider gehostet, da das RZ der Hochschule zu diesem Zweck keine VM zur Verfügung stellen konnte.
	...
	\subsection{Host Einrichtung}\label{sec:UmsetzungHostEinrichtung} %Mario
		...
		\subsubsection{Docker}\label{sec:DockerAufsetzen}
			Bevor die verschiedenen \nameref{itm:Container} konfiguriert werden können, muss zunächst der Docker Dienst installiert werden.
			
			Zunächst müssen allgemeine Updates geladen und installiert werden. Da Docker anschließend über \texttt{HTTPS} von einem Repository geladen wird, muss \texttt{apt} dies zunächst erlaubt werden. \cite{dockerInstall} 
			
			Dazu wird das Kommando aus \autoref{lst:aptRepositoryHTTPS} ausgeführt: 
\begin{lstlisting}[label={lst:aptRepositoryHTTPS},caption={allow \texttt{apt} to use repository over \texttt{HTTPS} \cite{dockerInstall}}]
...
\end{lstlisting}
	...
	Anschießend wird der GPG Key von der offiziellen Docker Website hinzugefügt.
\begin{lstlisting}[label={lst:addDockerGPGKey},caption={add Docker’s official GPG key \cite{dockerInstall}}]
$ curl -fsSL https://download.docker.com/linux/debian/gpg | 
	sudo apt-key add -
\end{lstlisting}

	Die Korrektheit des Schlüssels kann mit dem Befehl
\begin{lstlisting}[label={lst:addDockerGPGKey},caption={check Docker key \cite{dockerInstall}}]
$ sudo apt-key fingerprint 0EBFCD88
\end{lstlisting}
überprüft werden, indem der Fingerabdruck mitgegeben wird.

	Nun kann das eigentliche Repository über den nachfolgenden Befehl hinzugefügt werden. Dabei kann zwischen einer stabilen und Testversionen ausgewählt werden. 
\begin{lstlisting}[label={lst:addDockerGPGKey},caption={add Docker repository \cite{dockerInstall}}]
...
\end{lstlisting}
	...
	Sicherheitshalber wird vor der Installation nochmals ein Update für alle Pakete ausgeführt.
\begin{lstlisting}[label={lst:addDockerGPGKey},caption={update packages \cite{dockerInstall}}]
$ sudo apt-get update
\end{lstlisting}

	Nun kann die aktuelle Version von \textsf{Docker CE} und \textsf{containerd} installiert werden. Dies geschieht mit dem Befehl:
\begin{lstlisting}[label={lst:addDockerGPGKey},caption={install Docker CE and containerd \cite{dockerInstall}}]
$ sudo apt-get install docker-ce docker-ce-cli containerd.io
\end{lstlisting}

	Um zu prüfen ob der Dienst korrekt läuft, wird der folgende Befehl verwendet.
\begin{lstlisting}[label={lst:verifyDockerInstall},caption={verify Docker CE installation \cite{dockerInstall}}]
$ sudo docker run hello-world
\end{lstlisting}

	Nach diesen Schritten ist der Docker-Dienst nun einsatzbereit. Darin können jetzt verschiedenen Docker-\nameref{itm:Container} eingerichtet und ausgeführt werden.

		\subsubsection{Ordner Strukturen}\label{OrdnerStrukturen}
\begin{wrapfigure}{r}{0.35\textheight}
	\centering
    \includegraphics[width=0.5\textwidth]{hostTree.png}
...
\end{wrapfigure}			
	...
	\subsection{Eingangsserver}\label{sec:UmsetzungEingangsserver} %Manu
		...
		\subsubsection{Docker Konfiguration}\label{Mail-In-Container}
			...
			Es wird ein Full-Stack-Mailserver verwendet, dieser wird als Docker-Image eingebunden und konfiguriert. Um Mails empfangen und per Imap über einen Mail Client beziehen zu können benötigen wir die Ports:
		\begin{itemize}
			\item
				25 für TLS SMTP
			\item
...
% link markieren
		...
		\subsubsection{HTML-Seite}\label{WebsiteHTML-Seite}
			Da wir nur Crawlern das erhalten unserer Fakemails einfach ermöglichen wollten haben wir dann eine statische HTML-Seite mit Href-Einträgen für die Mails erstellt und diese Live gestellt, die Seite ist unter der Adresse https://babedibubip.de erreichbar.
