...
% Umdrehen. Erst sagen was erreichen, und dann wie
% "Ziel ist es die Analysen zu ermöglichen" (muss nicht sein, dass wir die erreichen)
Ein Mailserver soll zum einen \nameref{itm:Spam}-Nachrichten erhalten und zum anderen als offener Verteiler für Spammer verfügbar sein.
Zum einen soll der Inhalt von Spam-Nachrichten und zum anderen das Verhalten der Spammer analysiert werden.
Zur Durchführung wird hierzu ein Mailserver \nameref{itm:Honeypot} aufgesetzt.
\end{abstract}
...
% Fazit knüpft bei Einleitung an (bzw. Literaturüberblick - den haben wir nicht)
	...
	\\ % Absatz optional (bei uns evtl. eher nicht)
	% Warum schwierig
	% Zeitaufwendig
	% -> Kann man beim Ergebnis nochmal Stellungnahme nehmen oder hier nochmal verfeinern
	% -----
	% - Wirtschaftlichkeit
	% - Andere Beweggründe
	...
	\subsection{Vorgehensweise}\label{sec:EinleitungVorgehensweise}
		% Reihenfolge drehen: von Spam zur notwendigen VM gehen, nicht die VM erwähnen und dann Richtung Spam gehen
		Auf einer \nameref{itm:VirtuelleMaschine} werden die Dienste in \nameref{itm:Container}n aufgesetzt.
		Der Aufbau ist in \autoref{fig:Hierarchy} visualisiert.
		% TODO Dann nochmal darauf eingehen
		Die VM muss aus Sicherheitsgründen ins interne Netz abgeschottet sein.
		%%%Nach außen soll der Mail-Dienst zur Nutzung als \nameref{itm:Honeypot} verfügbar sein.
		...
		Den Eingangsserver betreffend gibt es folgende Aufgaben % umformulieren
		\begin{enumerate}
