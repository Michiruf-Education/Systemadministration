...
\section{Auswahl Lösung anhand Anforderungen}\label{sec:AuswahlLösungAnhandAnforderungen}
	...
	\subsection{Provider}\label{sec:AuswahlLösungProvider}
		...
		% TODO Mario: Link VM, Provider, Externer Link zur RZ Seite evtl.?
		...
		\subsubsection{Externer Provider}\label{sec:AuswahlLösungExterneProvider}
			...
			\begin{itemize}
				...
				% TODO Mario: Link zu 6.2, ist zwar eine vorwärts Referenz, aber scheiß drauf
			\end{itemize}		
	...
	\subsection{Maildienste}\label{sec:AuswahlLösungMailserver}
		...
		\subsubsection{Docker Mailserver-Image}\label{sec:AuswahlLösungVorkonfigurierterDockerMailserver}
			...
			% TODO Mario: Statt URL lieber eine Quelle angeben evtl.? Formatierung läuft so nicht wirklich gut mit Blocksatz
		...
		\subsubsection{OpenSMTPD}\label{sec:AuswahlLösungOpenSMTPD}
			Da der OpenSMTP Mailserver eine kompakte und übersichtliche Konfiguration bietet, ist dieser für den \nameref{itm:OpenRelay} geeignet.
			% TODO Mario: Link OpenSMTPD
		...
		\subsubsection{Dovecot}\label{sec:AuswahlLösungDovecot}
			% TODO Mario: Link überall: IMAP, POP3, Dovecot Quelle für Behauptungen
...
\section{Umsetzung}\label{sec:Umsetzung}
	...
	\subsection{Provider Registrierung}\label{sec:ProviderRegistrierung} 
		Wie bereits in \autoref{sec:AuswahlLösungProvider} erwähnt, wird die benötigte \nameref{itm:VirtuelleMaschine} für die Umsetzung des Projekts bei einem externen Provider gehostet.
		...
		% TODO Link Traffic; evtl. ebenfalls die Daten?
	...
	\subsection{Host Einrichtung}\label{sec:UmsetzungHostEinrichtung}
		...
		\subsubsection{DNS Einträge}\label{sec:DNSEinträge}
			...
			% TODO Mario: Link MX-Record + Def
			Zusätzlich zu den DNS-Einträgen für die Mailserver wird noch ein \texttt{www} Eintrag für die Website erstellt.
			% TODO Wir haben keinen WWW-Eintrag, die Webseite aber zu erwähnen ist aber geil
			% TODO Mario: Welche Webseite? Siehe ... hinzufügen

		\subsubsection{Docker}\label{sec:DockerAufsetzen}
			Bevor die verschiedenen \nameref{itm:Container} konfiguriert werden können, muss Docker installiert sein.
			...
			% TODO Mario: Ref auf Listing mit der Installation von Docker bzw. evtl. auf die Installationsseite von Docker, damit hier ein belegt ist wie das gemacht wird
	...
	\subsection{Eingangsserver}\label{sec:UmsetzungEingangsserver}
		...
		% TODO Mario: Link SSL, SQL, evtl. Authentizität?
		...
		\subsubsection{Mail-Container Konfiguration}\label{Mail-In-Container}
			...
			% TODO Ref 6.4.2
			Um Mails zu Empfangen und per IMAP über einen Mail-Client beziehen zu können werden die folgende Ports benötigt:
			\begin{itemize}
				\item 25 für TLS SMTP
				\item 143 TLS IMAP
				\item 587 TLS default mail submission port
				\item 993 SSL IMAP
			\end{itemize}
			Da für den Erhalt von Spam dieser nicht gefiltert werden soll, werden die Module Spamassassin, Clamav, Fail2ban, Postgrey und Spoof-Protection deaktiviert.
			% TODO Mario: Italic oder sowas (also Zitat-mäßig / Code-mäßig) für die Module
		...
		\subsubsection{SSL Konfiguration}\label{SSL-Container}
			...
			% TODO Mario: Link zu https://letsencrypt.org/
		...
		\subsubsection{Datenbank PHP-Script}\label{DB-Hook-Container}
			...
			Dieses holt sich per IMAP die Mails vom Eingangsserver und speichert diese in die SQL-Datenbank.
			Um das zu erreichen hat der Container eine Verlinkung zu den beiden anderen verwendeten Containern.
			Alle 30 Minuten wird das Script per cronjob aufgerufen.

			Das Script ist in \autoref{lst:PHP-Script} aufgeführt.
			% TODO Mario: PHP-Script in die Listings rein
			Die komplette Datenbank-Konfiguration ist in \autoref{lst:DB-Container} aufgeführt.
...
% TODO HERE WE ARE
		...
		\subsubsection{OpenSMTPD}\label{OpenRelayOpenSMTPD}
			...
			Die Ports die nach au{\ss}en im Netz verfügbar sind, werden dabei im Konfigurationsfile des Docker-\nameref{itm:Container} wie in \autoref{lst:RelayContainer} und \autoref{lst:RelayContainerDockerfile} definiert. Die Konfiguration für die \textsf{OpenSMTPD} wird in einer separaten
Datei angelegt.
%			Darin werden au{\ss}erdem die Plattenbereiche des \nameref{itm:Container}s mit den auf dem Host dafür vorgesehenen Speicher verlinkt und beim Start der VM gemounted. In diesem Bereich auf dem Hostsystem werden auch die Konfigurationsdateien für den \textsf{OpenSMTPD} abgelegt. 
			...
			In Kombination mit einem Docker-\nameref{itm:Container} werden die Konfigurationen teils etwas anders umgesetzt, wie es auf einem OpenBSD System der Fall ist. 
%			Bzw. wird die Kommunikation zwischen Mailserver Dienst im \nameref{itm:Container} und der \nameref{itm:Container} Konfiguration zum Host anders umgesetzt. 
Dies hat zur Folge das sich die Konfiguration eines \textsf{OpenSMTPD} innerhalb des Docker-\nameref{itm:Container}s schwieriger darstellt als anzunehmen war. Das Konfigurationsfile in \autoref{lst:smtpdConf} zeigt Teile der Konfigurationsversuche, die nicht zum gewünschten Erfolg geführt haben. 
